<template>
  <div class="leave-view h-full overflow-y-auto">
    <div class="p-4">
      <!-- Header -->
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-xs text-slate-600 mt-0.5">
            {{ totalRecords }} leave application{{ totalRecords !== 1 ? 's' : '' }} total
      ...
      <div class="relative isolate overflow-hidden rounded-xl bg-white/70 shadow-xl ring-1 ring-black/5 backdrop-blur">
        <!-- Empty State -->
        <div v-if="!isLoading && leaveApplications.length === 0" class="p-8 text-center">
          <svg class="mx-auto h-12 w-12 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <h3 class="mt-4 text-lg font-medium text-slate-900">No leave applications found</h3>
          <p class="mt-2 text-sm text-slate-600">
            {{ hasActiveFilters ? 'Try adjusting your filters' : 'No leave applications have been submitted yet' }}
          </p>
        </div>
        ...
        <!-- Pagination -->
        <div v-if="leaveApplications.length > 0" class="border-t border-slate-200 bg-slate-50/50 px-3 py-2">
...
import LeaveTable from './leave/LeaveTable.vue'
...
const handleEdit = async (record) => {
  const result = await getLeaveById(record.id)
  if (result.success) {
    if (permissions.value.canSelectEmployee) {
      fetchEmployees()
    }
...
const handleSubmit = async (payload) => {
  ...
  if (payload.mode === 'create') {
    result = await createLeave(payload.data)
  } else if (payload.mode === 'edit') {
    result = await updateLeave(selectedRecord.value.id, payload.data)
  }
...
onMounted(() => {
  loadPermissions()
  fetchLeaveTypes() // Load leave types on mount
