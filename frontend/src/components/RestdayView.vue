<template>
  <div class="restday-view h-full overflow-y-auto">
    <div class="p-4">
      <!-- Header -->
      <div class="flex items-center justify-between mb-4">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Rest Day Work Management</h1>
          <p class="text-xs text-slate-600 mt-0.5">
            {{ totalRecords }} restday application{{ totalRecords !== 1 ? 's' : '' }} total
      ...
      <div class="relative isolate overflow-hidden rounded-xl bg-white/70 shadow-xl ring-1 ring-black/5 backdrop-blur">
        <!-- Empty State -->
        <div v-if="!isLoading && restdayApplications.length === 0" class="p-8 text-center">
          <svg class="mx-auto h-12 w-12 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
          </svg>
          <h3 class="mt-4 text-lg font-medium text-slate-900">No restday applications found</h3>
          <p class="mt-2 text-sm text-slate-600">
            {{ hasActiveFilters ? 'Try adjusting your filters' : 'No restday applications have been submitted yet' }}
          </p>
        </div>
        ...
        <!-- Pagination -->
        <div v-if="restdayApplications.length > 0" class="border-t border-slate-200 bg-slate-50/50 px-3 py-2">
...
const handleEdit = async (record) => {
  const result = await getRestdayById(record.id)
  if (result.success) {
...
const handleSubmit = async (payload) => {
  ...
  if (payload.mode === 'create') {
    result = await createRestday(payload.data)
  } else if (payload.mode === 'edit') {
    result = await updateRestday(selectedRecord.value.id, payload.data)
  }
